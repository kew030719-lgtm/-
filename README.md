...
同一个面板还支持“用项目材料补充经历”：上传项目 ZIP、源码或 README 后，CareerRadar 只在本地静态读取文件结构和文本，先展示项目名、文件数、技术栈与可核验工作链路；用户点击确认后才会把材料归入“项目经历”，保留项目地址等原始证据，并重新生成当前岗位的定制简历。未确认的分析不会进入候选人画像或模型上下文。对应接口为 `POST/GET /api/resume-tailorings/{id}/project-uploads` 和 `POST /api/resume-tailorings/{id}/project-uploads/{upload_id}/confirm`。

技术版和商务版导出都会沿用候选人原简历的长页视觉骨架（橙色强调色、顶部个人信息、求职意向、教育经历、项目经历、相关技能），只根据目标岗位调整内容顺序和要点；不会再切换成通用 A4 模板。历史草稿重新导出时，系统会从原始证据恢复被 OCR 拆散的项目名称、日期、项目地址和学校信息，且不会把内部占位标题写入成品。

定向简历生成后会保存结构化验收报告。报告把每项检查标记为 `PASS`、`WARN` 或 `FAIL`：证据覆盖、重复要点、经历要点数量、内部占位标题和一页优先策略属于生成检查；DOCX/PDF 实际文件可打开、页数（最多两页）、乱码、内部证据 ID、联系方式和 ATS 文本层属于导出检查。未覆盖的岗位要求会显示为真实缺口提醒，不会被模型伪装成已掌握能力；存在 `FAIL` 时导出任务不会标记为成功，也不会提供“已通过”的下载链接。PDF 文本层优先用 `pypdf`，必要时用 PyMuPDF 复核，报告同时返回给 API 和前端。

上传 PDF 时优先读取文件自身的文字层；扫描版 PDF 会在本地渲染页面，再发送给用户配置的视觉模型逐字识别，最多支持 8 页。视觉模型与文本模型共用服务地址和安全存储的密钥，但使用独立的模型名称；只有本次上传的页面图片会发送给视觉模型，岗位数据库及其他历史数据不会随 OCR 请求发送。

遇到登录页、验证码、白屏或未知页面结构时，任务进入 `NEEDS_MANUAL_INPUT`。用户在专用标签页完成登录或验证后，点击扩展中的“继续任务”，从原队列位置恢复。助手不调用 Cookie API；只提取任务标签页的页面内容，上传前移除输入框、内嵌框架和非结构化脚本。采集完成后后端自动启动分析，关闭扩展弹窗不影响采集。
...
- 当前自动验证结果为：后端 `179 passed, 2 skipped`，扩展 Node 测试通过，前端类型检查、23 项测试及生产构建通过。真实站点的成功数、总耗时、P50/P95、重复数和暂停原因需要按站点另行记录。
...
- `career_radar/resume_qa.py`：生成后质量检查和导出文件/ATS 文本层验收；不依赖 LaTeX 或求职信模板。
...
定向简历接口从 `POST /api/target-jobs` 和 `POST /api/resume-tailorings` 开始。已采集岗位直接使用数据库中的岗位快照：可从岗位排名点击“为这个岗位修改简历”，也可在定向简历页面从已采集岗位中选择，不需要再次复制链接；链接和粘贴 JD 仅用于数据库之外的岗位。项目材料上传、分析和确认也位于这个“06 定向简历”面板，而不是原始简历上传区。追问确认后由异步任务生成草稿。草稿返回 `quality_report`，包含相关性、具体性、结构、简洁度、证据覆盖、重复项、预计页数、未覆盖要求以及结构化 `checks`。导出记录返回 `qa_report`，包括实际页数、ATS 文本层状态和每项修复建议；前端会显示通过、提醒和不合格项目，存在不合格项目时不提供下载。经历条目的标题与日期也必须引用当前经历的证据。`POST /api/resume-drafts/{id}/versions` 保存网页编辑的新版本并重新执行事实/质量检查，`POST /api/resume-drafts/{id}/exports` 生成两套 DOCX/PDF，下载文件仅从本地 `data/exports` 返回。模型生成的每条内容都必须引用当前候选人的证据；未经证实的技能、数字、机构、日期或跨经历拼接会使任务进入 `FAILED_VALIDATION`。脱敏的十组定向简历回归样本位于 `evaluation/tailoring_cases.json`，人工盲评达到 80% 改善率前不得宣称发布验收通过。

定向简历中的技能和能力表述允许自然改写，不再要求每个词逐字出现在证据原文中；质量评审会把生成内容与该条目引用的证据逐条对照，并用 `UNSUPPORTED` 标记无法支持的能力后返工。数字、机构、日期、证据 ID 和跨经历归属仍由后端确定性校验，不能仅凭岗位要求推断候选人具备某项技能。

用户在缺失事实追问中补充的“项目、程序、系统、仓库或爬虫”经历，会按回答中的真实短语归入“项目经历”，项目地址保留在同一项目证据中；没有明确项目信号的回答只保留为“补充经历”。旧版本里的“用户确认的补充经历”会在重新生成时迁移，内部占位标题不会进入最终简历。

“用户确认的补充经历”是系统内部的占位标题，不是候选人事实；它只用于承载用户补充的证据，不会要求这几个字出现在补充事实原文中。

定向简历的规划、正文改写和质量评审分别使用独立的输出额度。规划与评审也会为推理模型预留足够的内部思考空间，避免在尚未生成结构化结果前触发较小的默认 token 上限；该额度只影响定向简历阶段，不会放大全局对话的输出长度。

模型偶尔会超过提示词规定的篇幅。后端会在 Pydantic 校验前确定性限制为最多 2 条概述、8 条技能、10 段经历且每段 4 条要点；超出的尾部内容被舍弃，不会因为单段返回 5 条而让整份简历生成失败。
...
支持 BOSS 直聘、智联招聘、前程无忧，首版职位方向聚焦 AI、开发和数据类校招岗位，城市为北京、上海、深圳、广州、杭州、成都、武汉、南京、西安、苏州。不包含自动投递、自动联系招聘者、多用户权限、代理池或验证码处理。扫描版 PDF 只通过用户配置的视觉模型识别，不提供本地大模型或验证码自动处理。源码版本的浏览器助手仍需 Chrome 开发者模式；公开版必须完成 Chrome Web Store 发布。站点页面结构变化时可能需要手动粘贴岗位描述。fixture 测试通过并不保证真实站点始终可访问。
