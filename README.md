...
助手每 30 秒检查任务并推进一次队列状态，页面导航间隔至少 10 秒，实际通常约 60 秒处理一个页面。Chrome 休眠可能进一步延迟。弹窗会显示当前连接的主程序地址；切换地址时自动清除旧实例的任务游标，主程序中找不到的旧任务也会被自动丢弃。队列和开关存放在扩展本地存储，可在后台工作线程重启后恢复；关闭浏览器期间不执行。取消任务后后端拒绝继续接收岗位。
...
**关于模型输出的一个已知限制（实测）**：当前配置的端点（`deepseek-v4-pro-0813`）能可靠处理扁平的小 schema（打招呼语、岗位方向推荐），但对嵌套较深的 schema 会返回残缺内容——面试准备原本要求每题 5 个字段，实测返回的 `category` 为空、`evidence_ids` 为空，`days` 为空，甚至出现 `{{placeholder}}` 这样的模板残片。因此 `InterviewQuestionOutput` 被刻意压平为三个字段，`why_asked` / `answer_hint` / `deliverable` 改由后端派生。

即便如此，模型路径仍可能返回不可用内容。系统的处理是**降级并说明**，不是失败也不是静默替换：
